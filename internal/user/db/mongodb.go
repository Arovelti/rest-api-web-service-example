...
func (d *db) FindOne(ctx context.Context, id string) (u user.User, err error) {
	...
	if err = result.Decode(&u); err != nil {
		return u, fmt.Errorf("failed to decode user (id:%s) from db %v", id, err)
	}
	return u, nil
}
...
func (d *db) FindAll(ctx context.Context) (u []user.User, err error) {
	...
	if err := cursor.All(ctx, &u); err != nil {
		return u, fmt.Errorf("failed to read all documets from cursor: %v", err)
	}
	return u, nil
}
...
func (d *db) Update(ctx context.Context, user user.User) error {
	...
	if err != nil {
		return fmt.Errorf("failed to convert user ID to Obkject ID, ID: %s", user.ID)
	}
	filter := bson.M{"_id": objectID}

	userBytes, err := bson.Marshal(user)
	if err != nil {
		return fmt.Errorf("failed to marsha; user, error: %v", err)
	}
...
func (d *db) Delete(ctx context.Context, id string) error {
	...
	if err != nil {
		return fmt.Errorf("failed to convert user ID to Obkject ID, ID: %s", id)
	}
	filter := bson.M{"_id": objectID}
